...
class AICodexApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: AICodexHome(initialRoutePath: initialRoutePath, autoSignIn: autoSignIn),
    );
  }
}

class AICodexHome extends StatefulWidget {
  const AICodexHome({super.key, this.initialRoutePath, this.autoSignIn = false});
...
class _AICodexHomeState extends State<AICodexHome> {
  ...
  CopilotRoute get _route => _pilot.currentRoute ?? AICodexSpecs.initialRoute(explicitPath: widget.initialRoutePath, currentUri: Uri.base, presets: _presets);

  UxRouteSpec get _spec => AICodexSpecs.resolve(_route, presets: _presets);
  ...
  Future<void> _signInWithControllers() {
    return _signIn(username: _usernameController.text.trim(), password: _passwordController.text);
  }

  Future<bool> _signInWithMockCredentials() {
    return _signIn(username: Autopilot.mockUsername, password: Autopilot.mockPassword);
  }

  Future<bool> _signIn({required String username, required String password}) async {
    final applied = _pilot.applyMockAuth(username: username, password: password, notify: false);
    if (!applied) {
    ...
    final routePath = AICodexSpecs.initialPath(explicitPath: widget.initialRoutePath, currentUri: Uri.base, presets: presets);
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Text('Mock credential: admin / admin', style: theme.textTheme.bodyMedium, textAlign: TextAlign.center),
                ...
                FilledButton(onPressed: _signInWithControllers, child: const Text('Sign In')),
              ],
            ),
          ),
        ),
  ...
  Widget _buildLoading() {
    return const Scaffold(
      body: Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[CircularProgressIndicator(), SizedBox(height: 16), Text('Loading aicodex...')]),
      ),
    );
  }
  ...
  Widget _buildReady(BuildContext context) {
    ...
    final selectedIndex = presets.indexWhere((UxRouteSpec preset) => preset.path == route.path);
    ...
    return Scaffold(
          ...
          NavigationRail(
            ...
            destinations: presets
                .map<NavigationRailDestination>(
                  (UxRouteSpec preset) => NavigationRailDestination(icon: const Icon(Icons.folder_open_outlined), selectedIcon: const Icon(Icons.folder_open), label: Text(preset.title)),
                )
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Text(spec.subtitle, style: Theme.of(context).textTheme.bodyMedium),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),
                  Expanded(
                    child: GenUx.buildPaper(spec: spec.paper, autopilot: _pilot, optionalId: spec.optionalId),
                  ),
                ],
              ),
            ),
      ...
      bottomNavigationBar: BottomAppBar(child: Row(children: <Widget>[Text('Route: ${route.path}'), const Spacer(), Text('AICodex:${AICodexSpecs.appMeta}/${AppMeta.f}/${AppMeta.v}')])),
    );
  }
}
