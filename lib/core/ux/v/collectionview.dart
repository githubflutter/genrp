...
import 'package:genrp/core/ux/v/datatableview.dart';
import 'package:genrp/core/ux/v/empty.dart';
import 'package:genrp/core/ux/v/gridview.dart';
import 'package:genrp/core/ux/v/listview.dart';
  ...
  Widget build(BuildContext context) {
    ...
    if (children.isEmpty && columns.isEmpty && rows.isEmpty) {
      return UxEmptyView(
        i: i,
        autopilot: autopilot,
        p: resolvedTitle.isNotEmpty ? resolvedTitle : 'Empty collection',
      );
    }
  ...
  List<Widget> _buildListChildren(BuildContext context) {
    if (children.isNotEmpty) {
      return children.indexed
          .map<Widget>(
            ((int, Widget) record) =>
                _wrapSelectable(context, record.$1, child: record.$2),
          )
          .toList(growable: false);
    }
    return rows.indexed
        .map<Widget>(
          ((int, List<Object?>) record) => _wrapSelectable(
            context,
            record.$1,
            child: _rowListTile(context, record.$2),
          ),
        )
        .toList(growable: false);
  }
    ...
    return rows.indexed
        .map<Widget>(
          ((int, List<Object?>) record) => _wrapSelectable(
            context,
            record.$1,
            child: _rowCard(context, record.$2),
          ),
        )
        .toList(growable: false);
  }

  Widget _wrapSelectable(
    BuildContext context,
    int index, {
    required Widget child,
  }) {
    final isSelected = selectedIndex == index;
    final selectionColor = UxTheme.colors(
      context,
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Ink(
          decoration: UxTheme.softPanelDecoration(
            context,
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        if (subtitleText.isNotEmpty) ...<Widget>[
          ...
          Text(subtitleText, style: UxTheme.bodyStyle(context)),
        ],
      ],
    );
  }
